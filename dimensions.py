...
def save_predicted_positions(predicted_positions, csv_filename):
...
csv_filename = "Prueba2.csv"
real_positions = read_real_positions(csv_filename)

vidcap = cv2.VideoCapture("videos/Prueba2.MP4")
...
cv2.createTrackbar("L - V", "Trackbars", 42, 255, nothing)
...
save_predicted_positions(transformed_predicted_positions, "PredictedPositions.csv")

# Calculate and print the MAD for the left, middle, and right lanes
left_mad = calculate_mad(np.array([x for x, y in predicted_positions["left"]]))
middle_mad = calculate_mad(np.array([x for x, y in predicted_positions["middle"]]))
right_mad = calculate_mad(np.array([x for x, y in predicted_positions["right"]]))

print(f"Left MAD: {left_mad}")
print(f"Middle MAD: {middle_mad}")
print(f"Right MAD: {right_mad}")